import { angleBetween, dist, sub } from '../utils/geometry';

export function line([p0, ...points]: string[]) {
  return [['M', ...p0], ...points.map(p => ['L', ...p])];
}

export function area(points: string[]) {
  return [...line(points), ['Z']];
}

export function sector([c, p0, p1, p2, p3]: number[][]) {
  const r = dist(c, p0);
  const r1 = dist(c, p2);
...
export function ring([c, [r1, r2]]: number[][]) {
  const [cx, cy] = c;
  const p0 = [cx, cy - r2];
  const p1 = [cx, cy + r2];
  const p2 = [cx, cy + r1];
